...
export default function SuperAdmin() {
    ...
    const [activeTab, setActiveTab] = useState<WithdrawalRequest["status"]>("pending");
    const [alert, setAlert] = useState<MessageState | null>(null);
    ...
    const handleBroadcast = async () => {
        ...
        try {
            ...
            showAlert({ type: "success", text: "Broadcast started!" });
            setBroadcastMessage("");
        } catch (err) {
            console.error(err);
            showAlert({ type: "error", text: "Failed to send broadcast." });
        } finally {
    ...
    const handleAddAdmin = async () => {
        ...
        try {
            ...
            const response = await publicApi.post(`/api/admin/add?username=${username}`);
            const addedAdmin = response.data.admin;
            setNewAdmin("");
            showAlert({ type: "success", text: "Admin added successfully!" });
            setAdmins((prev) => [...prev, addedAdmin]);
        } catch (err) {
            console.error(err);
            showAlert({ type: "error", text: "Failed to add admin." });
        } finally {
    ...
    const handleDeleteAdmin = async (username: string) => {
        ...
        try {
            ...
            showAlert({ type: "success", text: "Admin removed successfully!" });
            setConfirmDelete(null);
            setAdmins((prev) => prev.filter((a) => a.username !== username));
        } catch (err) {
            console.error(err);
            showAlert({ type: "error", text: "Failed to delete admin." });
        } finally {
    ...
    // Alerts
    const showAlert = ({ type, text }: MessageState) => {
        setAlert({ type, text });
    ...
    const filtered = withdrawals
        ...
        .sort((a, b) => {
            if (activeTab === "pending") {
                ...
                return new Date(b.processed_at ?? 0).getTime() - new Date(a.processed_at ?? 0).getTime();
            }
        });
