...
async function hasJoinedChannel(userId: number) {
  try {
...
bot.start(async (ctx) => {
  try {
    ...
    logger.error(`❌ Error in /start handler: ${err instanceof Error ? err.message : String(err)}`);
    try {
  ...
  try {
    ...
    if (user?.referred_by) {
      try {
        ...
        logger.warn(
          `Could not notify referrer ${user.referred_by} about new referral: ${notifyErr instanceof Error ? notifyErr.message : String(notifyErr)}`,
        );
      }
    }
  } catch (err) {
    logger.error(`❌ Error in verify_join handler: ${err instanceof Error ? err.message : String(err)}`);
    try {
      await ctx.reply(
...
bot.action("show_referral", async (ctx) => {
  try {
    ...
    logger.error(`❌ Error showing referral link: ${err instanceof Error ? err.message : String(err)}`);
    try {
      await ctx.reply(
