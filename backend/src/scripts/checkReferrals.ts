...
async function isUserInChannel(userTelegramId: number) {
    try {
...
async function startReferralCheck() {

    if (isRunning) return;
    isRunning = true;

    logger.info("🌞 Starting optimized referral sweep...");
    ...
    try {
        ...
        while (hasMore) {
            ...
            logger.info(`Referrers length: ${referrers.length}`)
                ...
                const updatePromises = referrals.map((r) =>
                    limit(async () => {
                        try {
                            ...
                            logger.error(`Failed to update user in cron job ${r.telegram_id}: ${err instanceof Error ? err.message : String(err)}`);
                    ...
                    RETURNING id;`;
                        ...
                        if (cancelledItems.length > 0) {
                            ...
                            await bot.telegram.sendMessage(referrer.telegram_id, alertMsg, {
                                ...
                                logger.error(`Failed to notify user ${referrer.id} about withdraw cancelation: ${err.message || err}`);
                            });
                        }
                    } catch (dbErr) {
                        await client.query("ROLLBACK");
                        logger.error(`Failed to protect withdrawal for user ${referrer.id}: 
        ...
        logger.error(`❌ Daily sweep error: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
        client.release();
        isRunning = false;
